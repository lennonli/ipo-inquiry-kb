...
## supplement/ 补充法律意见书合集

顶层 `supplement/` 目录收录交易所公开披露的**补充法律意见书 824 份**（2001–2024，按文号年份子目录），与各年度 `cases/` 问询案例库互补（问询→回复→补充法律意见书全链条）。frontmatter 含 company/code/board/round/firm/doc_no/sign_date，索引为 `supplement/scripts/index.json`。检索：`kb_search.py search --kb supplement`。图片型扫描件（约一成多）正文无文本层，按元数据定位后回看原文件。详见 `supplement/README.md`。
