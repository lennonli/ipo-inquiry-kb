#!/usr/bin/env python3
"""把 listing-inquiry-digest/companies_2023/*.md 转换为知识库 cases/（带 frontmatter）+ 报告。
元数据（code/board/listing_date）以 state/companies_2023.json 与
state/neeq_listed_2023.json 为准；lawyer/inquiry_rounds/cutoff/tags 从正文提取。
标签归一化与 2026 版 migrate_20260825.py 同一 NORM 体系。
用法: python3 scripts/migrate_2023.py <digest_dir>
"""
import json, os, re, sys
from pathlib import Path

KB = Path(__file__).resolve().parent.parent
SRC = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/Users/licheng/Documents/zhipu/.zcode/workspace/default/listing-inquiry-digest/companies_2023")
STATE = SRC.parent / "state"
OUT = KB / "cases"
CUTOFF_DEFAULT = "2026-09-01"
...
def load_meta():
    """ticker/full_name/code -> 2023 年度公司元数据。"""
    meta = {}
    datasets = [
        (STATE / "companies_2023.json", False),
        (STATE / "neeq_listed_2023.json", True),
    ]
    for p, is_neeq in datasets:
        ...
        for c in rows:
            ...
            full_name = str(c.get("full_name", "") or "").strip()
            code = str(c.get("code", "") or "").strip()
            record = {
                "code": code,
                "board_raw": "新三板" if is_neeq else c.get("market", ""),
                "listing_date": c.get("listing_date", "") or c.get("listed_date", ""),
                "region": c.get("region", ""),
                "industry": c.get("industry", ""),
                "sponsor_broker": c.get("sponsor_broker", "") or c.get("sponsor", ""),
                "full_name": full_name,
            }
            # 正常文件名使用 ticker；同时保留 full_name/code 入口，覆盖批次 ticker
            # 为空或文件名采用其他稳定标识的简版公司。
            for key in (ticker, full_name, code):
                if key:
                    meta[key] = record
    return meta
...
def main():
    ...
    for fn in files:
        ...
        fm = [
            "---",
            f"company: {yaml_scalar(company)}",
            f"short: {yaml_scalar(stem)}",
            f"code: {yaml_scalar(code)}",
            f"board: {yaml_scalar(board)}",
            'layer: ""',
            f"listing_date: {yaml_scalar(listing_date)}",
            f"inquiry_rounds: {rounds if rounds else ''}",
            f"cutoff_date: {yaml_scalar(mc.group(1) if mc else CUTOFF_DEFAULT)}",
            f"lawyer: {yaml_scalar(lawyer)}",
